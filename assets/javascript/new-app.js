...
copyYmlButton.addEventListener('click', function(){
  ...
  let errorMessage = document.getElementById('yml-auto-complete-error');
  errorMessage.style.setProperty('display', 'none');
  let host, username, password, vizionEsAppId;

  try {
    host = vizionElasticUrl.value + ':443';
    let url = vizionElasticUrl.value.split('//');
    username = url[1].split(':')[0];
    url = url[1].split(':')[1];
    password = url.split('@')[0];
    url = url.split('@')[1];
    vizionEsAppId = url.split('.')[0];
  }
  catch(error) {
    console.log(error);
    errorMessage.style.setProperty('display', 'block');
    host = '<COMPLETE ME>';
    username = '<COMPLETE ME>';
    password = '<COMPLETE ME>';
    vizionEsAppId = '<COMPLETE ME>';
  }
  finally {

    let ymlTemplate = `###################### Filebeat Configuration Example #########################
    ...
    let ymlTextToCopy = document.getElementById('yml-text-to-copy');
    ymlTextToCopy.style.setProperty('display', 'block');
    ymlTextToCopy.value = ymlTemplate;
  }
})